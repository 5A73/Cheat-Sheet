...
```
- #### 認証付きプロキシ
`feroxbuster`で認証が必要なプロキシを使用してディレクトリのブルートフォースを行うには、以下のようにコマンドを実行します。
...
- `-u http://127.0.0.1`: ターゲットURLを指定します。ここでは、ローカルホストのHTTPサーバーをターゲットにしています。
- `--proxy "http://lance.friedman:o>WJ5-jD<5^m3@10.10.11.131:3128"`: プロキシのURLを指定します。プロキシURLには、以下の情報が含まれます。
  - `lance.friedman`: プロキシ認証に使用するユーザー名。
  - `o>WJ5-jD<5^m3`: プロキシ認証に使用するパスワード。特殊文字を含むパスワードは、適切にエスケープする必要がある場合があります。
  - `10.10.11.131`: プロキシサーバーのIPアドレス。
  - `3128`: プロキシサーバーのポート番号。
- `-t 20`: 同時接続数を20に設定します。これにより、`feroxbuster`は20個のスレッドを使用して並列にリクエストを送信します。

### 注意点

- **特殊文字の扱い**: パスワードに含まれる特殊文字（例: `>` や `<`）は、シェルで特別な意味を持つことがあるため、適切にエスケープする必要があります。エスケープが必要な場合は、バッククォート (`\`) を使用します。例えば、`>` は `\>` とエスケープします。
  
  例えば、`o>WJ5-jD<5^m3` のパスワードをエスケープする場合は、次のようにします。

  ```bash
  feroxbuster -u http://127.0.0.1 --proxy "http://lance.friedman:o\>WJ5-jD\<5^m3@10.10.11.131:3128" -t 20
  ```

- **プライバシーとセキュリティ**: コマンドラインで認証情報を渡すと、その情報がシェル履歴に残ることがあります。セキュリティ上のリスクを考慮して、履歴に残らないようにするか、使用後に履歴をクリアすることを検討してください。

このコマンドにより、指定されたプロキシを経由して、ローカルホストで実行されているHTTPサーバーに対するディレクトリブルートフォースを効率的に行うことができます。
